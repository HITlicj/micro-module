...
export class ScopedCSS {
  private static ModifiedTag = 'Symbol(style-modified)';
  ...
  private rewrite(rules: CSSRule[], prefix = '') {
    let css = '';
  ...
  private ruleStyle(rule: CSSStyleRule, prefix: string) {
    ...
    const splitIndex = cssText.indexOf('{') + 1;
    let cssK = cssText.substring(0, splitIndex);
    const cssV = cssText.substring(splitIndex);
    const getRewriteStr = (item: string, p: string, s: string) => {
      ...
      const ss = s.startsWith(' ') ? s.replace(/^ */, '') : s;
      return `${p}${prefix} ${ss}`;
    };
    if (cssK.includes(',')) {
      cssK = cssK.replace(/(^|,\n?)([^,]+)/g, getRewriteStr);
    } else {
      cssK = getRewriteStr(cssK, '', cssK);
    }
